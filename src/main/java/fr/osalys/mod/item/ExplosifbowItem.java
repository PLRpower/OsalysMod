...
public class ExplosifbowItem extends OsalysmodModElements.ModElement {

	@ObjectHolder("osalysmod:explosive_bow")
	...
	public static class ItemRanged extends Item {
		...
		public ItemRanged() {
			super(new Item.Properties().group(ExoliaItemGroup.tab).maxDamage(100));
		...
		public void onPlayerStoppedUsing(ItemStack itemstack, World world, LivingEntity entityLiving, int timeLeft) {
			if (!world.isRemote && entityLiving instanceof ServerPlayerEntity) {
				...
				if (true) {
					ItemStack stack = ShootableItem.getHeldAmmo(entity, e -> e.getItem() == ExplosifmunitionItem.block);

					if (stack == ItemStack.EMPTY) {
						for (int i = 0; i < entity.inventory.mainInventory.size(); i++) {
							ItemStack teststack = entity.inventory.mainInventory.get(i);
							if (teststack != null && teststack.getItem() == ExplosifmunitionItem.block) {
								stack = teststack;
								break;
							}
						}
					...
					if (entity.abilities.isCreativeMode || stack != ItemStack.EMPTY) {
						...
						ArrowCustomEntity entityarrow = shoot(world, entity, random, 0.5f, 0.1, 0);
						...
						if (entity.abilities.isCreativeMode) {
							...
							if (new ItemStack(ExplosifmunitionItem.block).isDamageable()) {
	...
	public static class ArrowCustomEntity extends AbstractArrowEntity implements IRendersAsItem {
		...
		public ItemStack getItem() {
			return new ItemStack(ExplosifmunitionItem.block);
		}
		...
		public void onEntityHit(EntityRayTraceResult entityRayTraceResult) {
			...
			ExplosifbowBulletHitsBlockProcedure.executeProcedure(Collections.emptyMap());
		}
